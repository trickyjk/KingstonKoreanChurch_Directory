...
def load_data():
    try:
        ...
        # [보정] Invalid base64 및 Incorrect padding 에러 해결
...
def member_dialog(member_data, row_index, sheet, mode="edit"):
    ...
    with st.form("member_form"):
        ...
        if st.form_submit_button("💾 구글 시트에 저장하기", type="primary"):
            final_photo = member_data.get('사진', '') # 기존 사진 유지
            updated_data['전화번호'] = format_phone_number(updated_data['전화번호'])
            
            headers = sheet.row_values(1)
            row_values = []
            for h in headers:
                if h == '사진': row_values.append(final_photo)
                else: row_values.append(updated_data.get(h, member_data.get(h, "")))
...
if df is not None:
    ...
    if print_mode:
        ...
        with c1: search = st.text_input("🔍 성도 검색 (이름이나 전화번호 입력)")
        with c2: 
            st.write(""); 
        ...
        for idx, row in f_df.iterrows():
            ...
            with cols[2]:
                if st.button("✏️ 수정", key=f"e_{idx}"):
                    member_dialog(row.to_dict(), idx, sheet, mode="edit")
            st.divider()
