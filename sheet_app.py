...
<style>
    ...
    .title-box {{
        background-color: white; padding: 30px; border-radius: 10px; margin-bottom: 20px;
        text-align: center; border: 1px solid #ddd; box-shadow: 0 2px 5px rgba(0,0,0,0.05);
    }}
    .print-header-text {{
        font-family: 'Nanum Myeongjo', serif; font-size: 42px; font-weight: bold;
        color: {TITLE_COLOR} !important; letter-spacing: 2px; margin-bottom: 15px;
    }}
    .print-header-line {{
        border-bottom: 3px double {TITLE_COLOR}; width: 80%; margin: 0 auto; opacity: 0.5;
    }}
    .print-logo-img {{ display: block; margin-left: auto; margin-right: auto; max-height: 120px; }}
    .print-card {{
...
# 3. 데이터 연결 (★ 이 부분이 똑똑해졌습니다!)
@st.cache_resource
def get_creds():
    scope = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
    
    # 1) 클라우드 환경인지 확인 (비밀 금고에 키가 있는지?)
    if "gcp_service_account" in st.secrets:
        creds = Credentials.from_service_account_info(st.secrets["gcp_service_account"], scopes=scope)
    # 2) 아니면 내 컴퓨터 환경인지 확인 (파일이 있는지?)
    else:
...
def generate_card_html(person, selected_cols):
    ...
    for col in selected_cols:
        val = person.get(col, '')
        if val: info_html += f'<div class="print-row"><span class="print-label">{col}:</span> {val}</div>'
    return f"""
...
if df is not None:
    ...
    if print_mode:
        if CHURCH_LOGO_URL:
            st.markdown(f'<div class="title-box"><img src="{CHURCH_LOGO_URL}" class="print-logo-img"></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="title-box"><div class="print-header-text">2026 킹스턴한인교회 주소록</div><div class="print-header-line"></div></div>', unsafe_allow_html=True)
        
        print_df = df.copy()
        addr_head_map = {}
        for idx, row in print_df.iterrows():
            addr = str(row.get('주소', '')).strip()
            if addr and addr not in addr_head_map: addr_head_map[addr] = row.get('이름', '')
